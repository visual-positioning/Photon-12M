...
# PATHS
# -------------------------------------------------
IMG_DIR = "/scratch/kalidas_5/Dataset/flickr8k/Images"
KARPATHY_JSON = "/scratch/kalidas_5/Dataset/flickr8k/dataset_flickr8k.json"
...
for item in data["images"]:
    if item["split"] != "test":
        continue
    fname = item["filename"]
    gt[fname] = [s["raw"] for s in item["sentences"]]
...
with torch.no_grad():
    for fname in tqdm(image_list, desc="Generating captions"):
        ...
        emb = img_model.encode_image(img_tensor)
...
print(f"Evaluating {len(preds)} images")

# -------------------------------------------------
# METRICS
# -------------------------------------------------
print("\n--- Flickr8k Test Results ---")
